...
(function () {
  ...
  function renderRuleEngine() {
    ...
    try {
      ...
      const histFull = Array.isArray(rb.equity_hist) ? rb.equity_hist
        ...
        .filter(Boolean) : [];
      const _rSecs = { '1h': 3600, '6h': 21600, '1d': 86400, '2d': 172800 }[state.reRange];   // undefined => all
  ...
  function renderLiveAccount() {
    ...
    if (!rb || !rb.live || !meta) { panel.hidden = true; return; }
    panel.hidden = false;
    const nPos = Object.keys(rb.positions || {}).length;
    const health = [
      ['Equity', fmtUSD(rb.equity), '', ''],
      ['Day open', meta.day_start_eq != null ? fmtUSD(meta.day_start_eq) : '—', '', 'UTC baseline'],
    ...
    // realized trades (async, cached 60s)
  ...
  function renderAll() {
    ...
    try { renderLiveAccount(); } catch (e) { try { console.error('live account panel', e); } catch (_) {} const lp = $id('la-panel'); if (lp) lp.hidden = true; }
