...
(function () {
  ...
  const state = {
    ...
    reRange: 'all',           // equity chart time window: 1h|6h|1d|2d|all (buttons in the chart header)
    dailyChart: null,
  ...
  const num = (v) => (typeof v === 'number' && !isNaN(v) ? v : null);   // module-scope: also used by handleRuleAction (was previously only local to renderRuleEngine → buttons threw ReferenceError)
  function fmtPx(x) {
    if (x == null || isNaN(x)) return '—';
    const a = Math.abs(x);
    let d;
  ...
  { const rb_ = $id('re-range-btns'); if (rb_) rb_.addEventListener('click', (e) => {   // equity chart time-range
    const btn = e.target.closest('.eq-range-btn'); if (!btn) return;
    state.reRange = btn.dataset.range || 'all';
    document.querySelectorAll('#re-range-btns .eq-range-btn').forEach((b) => b.classList.toggle('is-active', b === btn));
    if (state.status && state.status.rule_book) renderRuleEngine();
  }); }
  ...
  function renderRuleEngine() {
    ...
    try {
      ...
      panel.hidden = false;
      const nowS = Date.now() / 1000;
      { const _a = rb.live_meta && rb.live_meta.account; if (_a) fetchFunding(_a); }   // populate per-position funding cache
      ...
      const histFull = Array.isArray(rb.equity_hist) ? rb.equity_hist
        .map((pt) => (Array.isArray(pt) && pt.length >= 2 && num(pt[0]) != null && num(pt[1]) != null ? { ts: pt[0], equity: pt[1] } : null))
        .filter(Boolean) : [];
      const _rSecs = { '1h': 3600, '2h': 7200, '4h': 14400, '6h': 21600, '12h': 43200, '1d': 86400, '2d': 172800 }[state.reRange];   // undefined => all
      const hist = _rSecs ? histFull.filter((p) => p.ts >= (Date.now() / 1000) - _rSecs) : histFull;
      ...
      if (hist.length >= 2) {
        ...
        const chg = cur - hist[0].equity;
        const up = start == null ? cur >= (hist[0].equity) : cur >= start;
        if (swatch) swatch.className = 'swatch re-swatch-line' + (up ? '' : ' neg');
        if (rangeEl) rangeEl.textContent = `${(chg >= 0 ? '+' : '') + fmtUSD(chg)} · lo ${fmtUSD(lo)} · hi ${fmtUSD(hi)} · now ${fmtUSD(cur)} · ${hist.length} pts`;
      ...
      $id('re-live-cards').innerHTML = posEntries.length ? posEntries.map(([coin, p]) => {
        ...
        const stop = num(p.stop), tp = num(p.tp);
        const slUsd = usdAt(entry, stop, notion, p.side);                            // $ loss if SL hits
        const tpUsd = usdAt(entry, tp, notion, p.side);                              // $ profit if TP hits
        const fund = fundingFor(coin, num(p.opened_ts));                             // net funding since open (+earned/−paid)
        // R-progress: where mark sits between stop (−1R) and target (+2R). entry = 0R.
        let rNow = null, rPct = null;
        if (entry && stop && mark != null && entry !== stop) {
          rNow = (mark - entry) / Math.abs(entry - stop) * dir;
          rPct = Math.max(0, Math.min(100, (rNow + 1) / 3 * 100));
        }
        return `<div class="re-live-card ${cls}">
          ...
          ${rNow != null ? `<div class="re-rbar" title="progress from stop (−1R) to target (+2R)">
            <div class="re-rbar-track"><div class="re-rbar-entry"></div><div class="re-rbar-dot ${rNow >= 0 ? 'pos' : 'neg'}" style="left:${rPct}%"></div></div>
            <div class="re-rbar-lab"><span class="neg">−1R</span><span class="${pnlCls(rNow)}"><b>${rNow >= 0 ? '+' : ''}${rNow.toFixed(2)}R</b></span><span class="pos">+2R</span></div>
          </div>` : ''}
          ...
          ${(slUsd != null || tpUsd != null) ? `<div class="re-live-sub">${tpUsd != null ? `TP <span class="pos">${fmtUSD(tpUsd, true)}</span>` : ''}${(slUsd != null && tpUsd != null) ? ' · ' : ''}${slUsd != null ? `SL <span class="neg">${fmtUSD(slUsd, true)}</span>` : ''}</div>` : ''}
          ${fund != null ? `<div class="re-live-sub">funding <span class="${pnlCls(fund)}">${fmtUSD(fund, true)}</span> <span class="muted small">${fund >= 0 ? 'earned' : 'paid'}</span></div>` : ''}
          <div class="re-act">
            <button class="re-act-btn danger" data-act="close" data-coin="${esc(coin)}" title="Market-close this position">Close</button>
            <button class="re-act-btn" data-act="be" data-coin="${esc(coin)}" title="Move stop to entry (breakeven)">SL→BE</button>
            <button class="re-act-btn" data-act="trail" data-coin="${esc(coin)}" title="Set a new stop price">Trail SL</button>
            <button class="re-act-btn" data-act="move_tp" data-coin="${esc(coin)}" title="Set a new take-profit price">Move TP</button>
            <button class="re-act-btn" data-act="cancel_tp" data-coin="${esc(coin)}" title="Cancel take-profit (let it run)">Cancel TP</button>
            <button class="re-act-btn" data-act="extend_deadline" data-coin="${esc(coin)}" title="Extend the 48h time-stop">+ time</button>
          </div>
        </div>`;
      ...
      $id('re-pos-table').querySelector('tbody').innerHTML = posEntries.length ? posEntries.map(([coin, p]) => {
        ...
        const slUsd = usdAt(num(p.entry), num(p.stop), num(p.notional), p.side);
        const tpUsd = usdAt(num(p.entry), num(p.tp), num(p.notional), p.side);
        return `<tr>
          ...
          <td class="r neg" data-l="Stop">${fmtPx(p.stop)}${slUsd != null ? `<br><span class="neg small">${fmtUSD(slUsd, true)}</span>` : ''}</td>
          <td class="r pos" data-l="TP">${fmtPx(p.tp)}${tpUsd != null ? `<br><span class="pos small">${fmtUSD(tpUsd, true)}</span>` : ''}</td>
      ...
      $id('re-pending-table').querySelector('tbody').innerHTML = pend.length ? pend.map((o) => {
        ...
        const slUsd = usdAt(num(o.limit), num(o.stop), num(o.notional), o.side);
        const tpUsd = usdAt(num(o.limit), num(o.tp), num(o.notional), o.side);
        return `<tr>
          ...
          <td class="r neg" data-l="Stop">${fmtPx(o.stop)}${slUsd != null ? `<br><span class="neg small">${fmtUSD(slUsd, true)}</span>` : ''}</td>
          <td class="r pos" data-l="TP">${fmtPx(o.tp)}${tpUsd != null ? `<br><span class="pos small">${fmtUSD(tpUsd, true)}</span>` : ''}</td>
          ...
          <td class="r" data-l="Expires in">${left == null ? '—' : left <= 0 ? 'expired' : fmtAge(left)}
            <div class="re-act pend"><button class="re-act-btn" data-act="extend_expiry" data-coin="${esc(o.coin || '')}" title="Extend this resting limit's expiry">+ exp</button><button class="re-act-btn danger" data-act="cancel_pending" data-coin="${esc(o.coin || '')}" title="Cancel this resting limit">Cancel</button></div></td>
        </tr>`;
  ...
  // ---------------------------------------------------------------- render: LIVE ACCOUNT panel
  const laFills = { data: null, at: 0, acct: null };
  const laFunding = { data: null, at: 0, acct: null };
  // Funding payments on perps (HL userFunding). Negative usdc = you paid; positive = you received. Cached 60s.
  function fetchFunding(account) {
    if (!account) return Promise.resolve([]);
    if (laFunding.data && laFunding.acct === account && Date.now() - laFunding.at < 60000) return Promise.resolve(laFunding.data);
    const start = Date.now() - 45 * 86400 * 1000;   // last 45d is plenty for a live account days old
    return fetch('https://api.hyperliquid.xyz/info', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ type: 'userFunding', user: account, startTime: start }),
    }).then((r) => r.json()).then((f) => {
      laFunding.data = Array.isArray(f) ? f : []; laFunding.at = Date.now(); laFunding.acct = account; return laFunding.data;
    }).catch(() => laFunding.data || []);
  }
  // Net funding on a coin since `sinceTs` (position open). + = you were paid, − = you paid.
  function fundingFor(coin, sinceTs) {
    const arr = laFunding.data;
    if (!Array.isArray(arr) || !arr.length) return null;
    let net = 0, hit = false;
    for (const f of arr) {
      const d = f.delta || {};
      if (d.coin !== coin) continue;
      if (sinceTs && Number(f.time) / 1000 < sinceTs - 5) continue;
      net += Number(d.usdc || 0); hit = true;
    }
    return hit ? net : 0;
  }
  function fetchFills(account) {
    if (!account) return Promise.resolve([]);
    if (laFills.data && laFills.acct === account && Date.now() - laFills.at < 60000) return Promise.resolve(laFills.data);
    return fetch('https://api.hyperliquid.xyz/info', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ type: 'userFills', user: account }),
    }).then((r) => r.json()).then((f) => {
      laFills.data = Array.isArray(f) ? f : []; laFills.at = Date.now(); laFills.acct = account; return laFills.data;
    }).catch(() => laFills.data || []);
  }
  function pnlCls(v) { return v == null ? '' : v > 0 ? 'pos' : v < 0 ? 'neg' : ''; }
  // $ P&L if price reaches `level` from `entry`, given notional + side (long/short). +reward at TP, -loss at SL.
  function usdAt(entry, level, notional, side) {
    if (!entry || !level || !notional) return null;
    const dir = side === 'long' ? 1 : -1;
    return notional * (level - entry) / entry * dir;
  }
  ...
  function renderLiveAccount() {
    const rb = state.status && state.status.rule_book;
    const panel = $id('la-panel'); if (!panel) return;
    const meta = rb && rb.live_meta;
    if (!rb || !meta) { panel.hidden = true; return; }   // live_meta present => live account exists
    panel.hidden = false;
    const nPos = Object.keys(rb.positions || {}).length;
    const eqHist = rb.equity_hist || [];
    const inceptTs = eqHist.length ? eqHist[0][0] : (rb.start_ts || null);      // live tracking began
    const daysLive = inceptTs ? Math.max(0, (Date.now() / 1000 - inceptTs) / 86400) : null;
    const health = [
      ['Equity', fmtUSD(rb.equity), '', ''],
      ['Day open', meta.day_start_eq != null ? fmtUSD(meta.day_start_eq) : '—', '', 'trails +5%'],
      ['Today', fmtUSD(meta.day_pnl, true), pnlCls(meta.day_pnl), meta.day_pnl_pct != null ? fmtPct(meta.day_pnl_pct) : ''],
      ['All-time', fmtUSD(meta.all_time_pnl, true), pnlCls(meta.all_time_pnl), fmtPct(meta.all_time_pnl_pct)],
      ['Funding', '<span id="la-funding-val" class="muted">…</span>', '', 'net paid/earned'],
      ['Days live', daysLive != null ? daysLive.toFixed(1) + 'd' : '—', '', inceptTs ? new Date(inceptTs * 1000).toLocaleDateString(undefined, { month: 'short', day: 'numeric' }) : ''],
      ['Margin used', (meta.margin_pct || 0) + '%', '', fmtUSD(meta.margin_used)],
      ['Gross exp.', (meta.gross_pct || 0) + '%', '', fmtUSD(meta.gross_notional)],
      ['Free', fmtUSD(meta.available), '', ''],
      ['Positions', nPos + ' / ' + (meta.max_pos || 6), '', ''],
      ['Risk/trade', (meta.risk_pct || 5) + '%', '', ''],
    ];
    // ---- inception strip + next-scan countdown (isolated: never let it hide the whole panel) ----
    try {
      const eqNow = Number(rb.equity);
      const startEq = meta.all_time_pnl != null ? (eqNow - Number(meta.all_time_pnl)) : 300;
      const nextScan = (Math.floor(Date.now() / 1000 / 7200) + 1) * 7200;   // rule engine scans on the 2h boundary
      const incEl = $id('la-inception');
      if (incEl) incEl.innerHTML =
        `<span class="la-inc-tag">INCEPTION</span>` +
        (inceptTs ? `<span class="la-inc-i">${new Date(inceptTs * 1000).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })} · <b>${daysLive != null ? daysLive.toFixed(1) : '—'}d</b></span>` : '') +
        `<span class="la-inc-i">${fmtUSD(startEq)} → <b>${fmtUSD(eqNow)}</b></span>` +
        `<span class="la-inc-i ${pnlCls(meta.all_time_pnl)}"><b>${fmtUSD(meta.all_time_pnl, true)}</b> (${fmtPct(meta.all_time_pnl_pct)})</span>` +
        `<span class="la-inc-next">⏱ next setup scan in <b>${fmtAge(Math.max(0, nextScan - Date.now() / 1000))}</b></span>`;
    } catch (e) { try { console.error('inception strip', e); } catch (_) {} }
    $id('la-health').innerHTML = health.map(([label, val, cls, sub]) =>
      `<div class="la-stat"><div class="la-stat-label">${label}</div><div class="la-stat-val ${cls}">${val}</div>` +
      (sub ? `<div class="la-stat-sub ${cls}">${sub}</div>` : '<div class="la-stat-sub"></div>') + `</div>`).join('');
    // breaker gauge
    const brk = $id('la-breaker'); const brkSt = $id('la-breaker-status');
    if (meta.day_start_eq == null) { brk.innerHTML = '<span class="muted small">no day baseline yet</span>'; brkSt.innerHTML = ''; }
    else {
      const cap = meta.breaker_cap_pct || 2; const dayPct = meta.day_pnl_pct || 0;
      const frac = Math.max(0, Math.min(1, (Math.max(-cap, Math.min(cap, dayPct)) + cap) / (2 * cap)));
      const near = dayPct <= -cap * 0.6;
      brkSt.innerHTML = meta.breaker_tripped ? '<span class="badge-trip">TRIPPED · no new entries today</span>' : '<span class="badge-armed">ARMED</span>';
      brk.innerHTML =
        `<div class="brk-track"><div class="brk-mid"></div>` +
        `<div class="brk-marker ${dayPct >= 0 ? 'pos' : near ? 'warn' : 'neg'}" style="left:${(frac * 100).toFixed(1)}%"></div></div>` +
        `<div class="brk-labels"><span class="neg">−${cap}% stop · ${fmtUSD(meta.day_start_eq * (1 - cap / 100))}</span><span>open ${fmtUSD(meta.day_start_eq)}</span><span class="pos">+${cap}%</span></div>` +
        `<div class="brk-now ${pnlCls(meta.day_pnl)}">Today ${fmtUSD(meta.day_pnl, true)} · ${fmtPct(dayPct)} — new entries stop if equity hits ${fmtUSD(meta.day_start_eq * (1 - cap / 100))}</div>` +
        `<div class="brk-ratchet">⤴ Profit-ratchet ON — at +5% (${fmtUSD(meta.day_start_eq * 1.05)}) the floor re-baselines up and banks the gain</div>`;
    }
    // auto-bucket
    const posC = new Set(Object.keys(rb.positions || {})); const pendC = new Set((rb.pending || []).map((p) => p.coin));
    $id('la-bucket').innerHTML = (meta.bucket || []).length
      ? meta.bucket.map((c) => `<span class="la-coin ${posC.has(c) ? 'in-pos' : pendC.has(c) ? 'in-pend' : ''}">${c}${posC.has(c) ? ' ●' : pendC.has(c) ? ' ○' : ''}</span>`).join('')
      : '<span class="muted small">rebuilding…</span>';
    // next weekly bucket re-selection (week = floor(now / 7d) in UTC)
    const wkNext = (Math.floor(Date.now() / 1000 / (7 * 86400)) + 1) * 7 * 86400;
    const bref = $id('la-bucket-refresh'); if (bref) bref.textContent = 'refreshes in ' + fmtAge(wkNext - Date.now() / 1000);
    // realized trades + funding (async, cached 60s)
    if (meta.account) fetchFills(meta.account).then((fills) => renderTrades(fills));
    if (meta.account) fetchFunding(meta.account).then((fund) => {
      const fv = $id('la-funding-val'); if (!fv) return;
      const net = (fund || []).reduce((a, f) => a + Number((f.delta && f.delta.usdc) || 0), 0);
      fv.textContent = fmtUSD(net, true); fv.className = pnlCls(net);   // + = received, − = paid
    });
  }
  function renderTrades(fills) {
    const el = $id('la-trades'); const statsEl = $id('la-stats'); if (!el) return;
    const closed = (fills || []).filter((f) => Number(f.closedPnl) !== 0);
    if (!closed.length) { el.innerHTML = '<div class="empty-state compact"><div class="empty-sub">No closed trades yet — both entries are still resting (no round-trip completed). Fills will appear here with live win-rate &amp; P&amp;L.</div></div>'; statsEl.innerHTML = ''; return; }
    const pnls = closed.map((f) => Number(f.closedPnl));
    const wins = pnls.filter((p) => p > 0).length; const total = pnls.reduce((a, b) => a + b, 0);
    const gW = pnls.filter((p) => p > 0).reduce((a, b) => a + b, 0); const gL = Math.abs(pnls.filter((p) => p < 0).reduce((a, b) => a + b, 0));
    const best = Math.max(...pnls), worst = Math.min(...pnls);
    statsEl.innerHTML =
      `<span class="la-stat-inline">Trades <b>${pnls.length}</b></span>` +
      `<span class="la-stat-inline">Win <b>${Math.round(wins / pnls.length * 100)}%</b></span>` +
      `<span class="la-stat-inline">Net <b class="${pnlCls(total)}">${fmtUSD(total, true)}</b></span>` +
      `<span class="la-stat-inline">PF <b>${gL > 0 ? (gW / gL).toFixed(2) : '∞'}</b></span>` +
      `<span class="la-stat-inline">Best <b class="pos">${fmtUSD(best, true)}</b></span>` +
      `<span class="la-stat-inline">Worst <b class="neg">${fmtUSD(worst, true)}</b></span>`;
    el.innerHTML =
      `<table class="tbl num-tbl la-tbl"><thead><tr><th>Time</th><th>Coin</th><th>Dir</th><th class="r">Price</th><th class="r">P&amp;L</th></tr></thead><tbody>` +
      closed.slice(0, 20).map((f) => {
        const pnl = Number(f.closedPnl); const dir = (f.dir || '');
        return `<tr><td class="muted small">${fmtClock(f.time)}</td><td>${f.coin}</td>` +
          `<td class="${dir.toLowerCase().indexOf('long') >= 0 ? 'pos' : 'neg'} small">${dir}</td>` +
          `<td class="r">${f.px}</td><td class="r ${pnlCls(pnl)}">${fmtUSD(pnl, true)}</td></tr>`;
      }).join('') + `</tbody></table>`;
  }
  ...
  function renderAll() {
    ...
    try { renderLiveAccount(); } catch (e) { try { console.error('live account panel', e); } catch (_) {} /* keep panel visible: it may be partly rendered, better than vanishing */ }
    ...
    renderExitQuality();
    renderConfig();
    applyDashView();   // unified view gate: hide LLM panels in "rule engine" view (must run AFTER each panel's own render)
    markScrollable();
    listeners.status.forEach((cb) => { try { cb(state.status); } catch (_) { /* admin hook errors must not break the dashboard */ } });
  }

  // ---- unified dashboard view toggle: rule engine (live) / LLM book (paper) / both ----
  // The LLM cycles continuously even when it isn't the focus, so there's no clean "off" signal; this gives an
  // explicit, remembered choice. Default = rule engine (your live real-money trader), LLM hidden until asked for.
  const LLM_PANEL_SEL = ['.chart-panel', '#brief-panel', '.positions-panel', '#shadowtr-panel', '.feed-panel', '#perf-panel', '.risk-panel'];
  const RULE_PANEL_SEL = ['#rule-engine-panel', '#la-panel'];
  try { state.dashView = localStorage.getItem('dashView') || 'rule'; } catch (_) { state.dashView = 'rule'; }
  function applyDashView() {
    const v = state.dashView || 'rule';
    const showLLM = (v === 'llm' || v === 'both');
    const showRule = (v === 'rule' || v === 'both');
    // Only force-HIDE panels the current view excludes; panels the view includes keep whatever their own render set.
    if (!showLLM) LLM_PANEL_SEL.forEach((s) => document.querySelectorAll(s).forEach((el) => { el.hidden = true; }));
    if (!showRule) RULE_PANEL_SEL.forEach((s) => document.querySelectorAll(s).forEach((el) => { el.hidden = true; }));
    // status-panel is LLM/goal content (equity, LLM cost, multiple-vs-target, 30-day horizon) — hide those tiles in
    // rule view, but KEEP the agent-down alert. Show the panel shell only when there's an alert to display.
    ['.v3-note', '.glance', '.goal-row'].forEach((s) => document.querySelectorAll('#status-panel ' + s).forEach((el) => { el.hidden = !showLLM; }));
    const sp = $id('status-panel'), al = $id('alerts');
    if (sp) sp.hidden = !showLLM && !(al && al.children.length);
    document.querySelectorAll('#dash-view-bar .dvb-btn').forEach((b) => b.classList.toggle('is-active', b.dataset.view === v));
  }
  document.querySelectorAll('#dash-view-bar .dvb-btn').forEach((b) => b.addEventListener('click', () => {
    state.dashView = b.dataset.view;
    try { localStorage.setItem('dashView', state.dashView); } catch (_) { /* private mode */ }
    if (state.status) renderAll(); else applyDashView();
  }));
  ...
  // ---- manual position/pending action buttons: styled modal + presets + toast (no native prompt/alert) ----
  function ensureActionModal() {
    if ($id('act-modal-backdrop')) return;
    const el = document.createElement('div');
    el.className = 'act-modal-backdrop'; el.id = 'act-modal-backdrop'; el.hidden = true;
    el.innerHTML = '<div class="act-modal" role="dialog" aria-modal="true">'
      + '<div class="act-modal-head"><h3 id="act-modal-title"></h3><button class="act-modal-x" id="act-modal-x" type="button" aria-label="Close">×</button></div>'
      + '<div class="act-modal-body" id="act-modal-body"></div>'
      + '<div class="act-modal-foot"><button class="act-modal-cancel" id="act-modal-cancel" type="button">Cancel</button><button class="act-modal-ok" id="act-modal-ok" type="button">Confirm</button></div></div>';
    document.body.appendChild(el);
  }
  function actionPrompt(opts) {
    ensureActionModal();
    return new Promise((resolve) => {
      const bd = $id('act-modal-backdrop');
      $id('act-modal-title').textContent = opts.title || '';
      const f = opts.field;
      $id('act-modal-body').innerHTML =
        (opts.context ? `<div class="act-context">${opts.context}</div>` : '')
        + (f ? `<div class="act-field"><label>${esc(f.label)}</label>`
             + `<input type="number" step="any" inputmode="decimal" id="act-input" value="${f.value != null ? f.value : ''}">`
             + ((f.presets && f.presets.length) ? `<div class="act-presets">${f.presets.filter((p) => p.value != null && isFinite(p.value)).map((p) => `<button type="button" class="act-preset" data-v="${p.value}">${esc(p.label)}</button>`).join('')}</div>` : '')
             + '<div class="act-err" id="act-err" hidden></div></div>' : '')
        + (opts.note ? `<div class="act-note">${opts.note}</div>` : '');
      const ok = $id('act-modal-ok'); ok.textContent = opts.confirmLabel || 'Confirm'; ok.classList.toggle('danger', !!opts.danger);
      bd.hidden = false;
      const input = $id('act-input'); if (input) setTimeout(() => { input.focus(); input.select(); }, 30);
      $id('act-modal-body').querySelectorAll('.act-preset').forEach((b) => { b.onclick = () => { if (input) { input.value = b.dataset.v; input.focus(); } }; });
      const done = (v) => { bd.hidden = true; document.removeEventListener('keydown', onKey); resolve(v); };
      const submit = () => {
        if (!input) return done(true);
        const v = parseFloat(input.value); const err = $id('act-err');
        if (!isFinite(v) || v <= 0) { if (err) { err.textContent = 'Enter a valid positive number.'; err.hidden = false; } return; }
        done(v);
      };
      ok.onclick = submit;
      $id('act-modal-cancel').onclick = () => done(null);
      $id('act-modal-x').onclick = () => done(null);
  ...
  function toast(msg, kind) {
    let t = $id('act-toast');
    if (!t) { t = document.createElement('div'); t.id = 'act-toast'; t.className = 'act-toast'; document.body.appendChild(t); }
    t.textContent = msg; t.className = 'act-toast show ' + (kind || '');
    clearTimeout(t._h); t._h = setTimeout(() => { t.className = 'act-toast ' + (kind || ''); }, 3400);
  }
  const RA_LABELS = { close: 'Close position', be: 'Move SL → breakeven', trail: 'Trail stop', move_tp: 'Move take-profit', cancel_tp: 'Cancel take-profit', cancel_pending: 'Cancel resting limit', extend_deadline: 'Extend time-stop', extend_expiry: 'Extend expiry' };
  async function handleRuleAction(act, coin) {
    if (!act || !coin) return;
    if (!window.ruleAction || !(window.adminReady && window.adminReady())) {
      await actionPrompt({ title: 'Admin sign-in required', note: 'These buttons send real orders to your live account, so they’re admin-only. Click the shield 🛡 (Admin) button in the top bar, sign in with Google, then try again.', confirmLabel: 'Got it' });
      return;
    }
    const rb = state.status && state.status.rule_book;
    const pos = rb && rb.positions && rb.positions[coin];
    const isLong = !pos || pos.side === 'long'; const dir = isLong ? 1 : -1;
    const mark = pos && num(pos.mark), entry = pos && num(pos.entry), curStop = pos && num(pos.stop), curTp = pos && num(pos.tp);
    const rr = (entry && curStop) ? Math.abs(entry - curStop) : null;
    const ctx = pos ? `${esc(coin)} · mark ${fmtPx(mark)} · entry ${fmtPx(entry)}${curStop != null ? ' · SL ' + fmtPx(curStop) : ''}${curTp != null ? ' · TP ' + fmtPx(curTp) : ''}` : esc(coin);
    let opts, params = {};
    if (act === 'trail') {
      const presets = [{ label: 'Breakeven', value: entry }];
      if (mark != null) presets.push({ label: isLong ? '1% under mark' : '1% over mark', value: +(mark * (1 - dir * 0.01)).toPrecision(6) });
      if (rr && entry) presets.push({ label: 'Lock +1R', value: +(entry + dir * rr).toPrecision(6) });
      opts = { title: `Trail stop — ${coin}`, context: ctx, field: { label: 'New stop-loss price', value: curStop != null ? curStop : entry, presets }, confirmLabel: 'Move stop' };
    } else if (act === 'move_tp') {
      const base = entry || mark;
      const presets = base ? [5, 10, 20].map((p) => ({ label: (isLong ? '+' : '−') + p + '%', value: +(base * (1 + dir * p / 100)).toPrecision(6) })) : [];
      opts = { title: `Move take-profit — ${coin}`, context: ctx, field: { label: 'New take-profit price', value: curTp != null ? curTp : (base ? +(base * (1 + dir * 0.10)).toPrecision(6) : ''), presets }, confirmLabel: 'Move TP' };
    } else if (act === 'extend_deadline' || act === 'extend_expiry') {
      const what = act === 'extend_deadline' ? 'time-stop' : 'limit expiry';
      const curDl = act === 'extend_deadline' ? (pos && num(pos.deadline_ts)) : null;
      const ctxTxt = curDl ? `Currently expires in ${fmtAge(curDl - Date.now() / 1000)} — this ADDS on top (won’t shorten it).` : `Adds time on top of the current ${what}.`;
      opts = { title: `Extend ${what} — ${coin}`, context: ctxTxt, field: { label: 'Hours to add', value: act === 'extend_deadline' ? 24 : 6, presets: [6, 12, 24, 48].map((h) => ({ label: '+' + h + 'h', value: h })) }, confirmLabel: 'Add time' };
    } else {
      opts = { title: `${RA_LABELS[act] || act} — ${coin}`, context: ctx, note: 'This sends a real action to your LIVE account.', confirmLabel: RA_LABELS[act] || 'Confirm', danger: act === 'close' || act === 'cancel_pending' };
    }
    const val = await actionPrompt(opts);
    if (val == null) return;
    if (act === 'trail' || act === 'move_tp') params.price = val;
    if (act === 'extend_deadline' || act === 'extend_expiry') params.hours = val;
    toast(`Sending ${RA_LABELS[act]} ${coin}…`);
    window.ruleAction(act, coin, params)
      .then(() => toast(`✓ ${RA_LABELS[act]} ${coin} queued — executes in ~10s`, 'ok'))
      .catch((e) => toast(`✗ Failed: ${(e && (e.detail || e.message)) || e}`, 'err'));
  }
  // Document-level delegation (robust on mobile; also handles buttons rendered after load). One handler, guarded
  // against double-fire from touch->click by a short debounce.
  let _lastAct = 0;
  function _onActClick(e) {
    const btn = e.target && e.target.closest && e.target.closest('.re-act-btn');
    if (!btn) return;
    e.preventDefault(); e.stopPropagation();
    const nowMs = Date.now(); if (nowMs - _lastAct < 400) return; _lastAct = nowMs;
    Promise.resolve().then(() => handleRuleAction(btn.dataset.act, btn.dataset.coin))
      .catch((err) => { try { console.error('rule-action handler', err); toast('✗ ' + ((err && err.message) || err), 'err'); } catch (_) { /* noop */ } });
  }
  document.addEventListener('click', _onActClick);
